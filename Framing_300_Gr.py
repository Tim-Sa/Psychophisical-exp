...
last_frameN_midle = frames_low+1
...
def hex_in_hight_speed(frameN):
    ...
    result = max_diametr-frameN*2*100/frames_hight #formula of poligon size for decreasing at high speed
    return result
...
def hex_out_hight_speed(frameN):
    ...
    result = min_diametr+frameN*2*100/frames_hight #formula of poligon size for increasing at high speed
    return result

def hex_in_midle_speed(frameN):
    global in_midle_counter  # count of using this func
    in_midle_counter += 1

    if in_midle_counter == repeats * last_frameN - 1:  # if using-num is equal to repeats - don't use this function anymore (delete from list of using funcs)
        functions_of_hex.remove(hex_in_midle_speed)

    result = max_diametr - frameN *2* 100 / frames_midle  # formula of poligon size for decreasing at high speed
    return result

def hex_out_midle_speed(frameN):
    global out_midle_counter
    out_midle_counter += 1

    if out_midle_counter == repeats * last_frameN - 1:
        functions_of_hex.remove(hex_out_midle_speed)

    result = min_diametr + frameN*2 * 100 / frames_midle  # formula of poligon size for increasing at high speed
    return result
...
def hex_in_low_speed(frameN):
    ...
    result = max_diametr-frameN*2*100/frames_low#formula of poligon size for decreasing at low speed
    return result
...
def hex_out_low_speed(frameN):
    ...
    result = min_diametr+frameN*2*100/frames_low#formula of poligon size for increasing at low speed
    return result
...
start_functions_of_hex =[hex_in_hight_speed, hex_out_hight_speed, hex_in_midle_speed, hex_out_midle_speed, hex_in_low_speed, hex_out_low_speed]
...
win = visual.Window(
    ...
    #!!THERE YOU NEED WRITE NAME OF YOUR MONITOR!!
    monitor='Name_of_Monitor', color=[0,0,0], colorSpace='rgb',
...
if expInfo['frameRate'] != None:
    ...
    frameDur = 1.0 / 300.0 # could not measure, so guess
...
#!#########################################################################################################################################################
    ...
    while continueRoutine:
        ...
        #!######################################--CHANGING OF FRAME AND POLIGON PARAMS FOR DIFFERENT SPEEDS--################################################
            polygon.lineWidth = line
            ...
            if function_of_hex == hex_in_midle_speed or function_of_hex == hex_out_midle_speed: #IF SPEED IS LOW - TAKE THIS PARAMS:
                last_frameN = last_frameN_midle
    ...
    elif function_of_hex == hex_out_midle_speed:
        out_midle_answers.append(key_resp_2.keys)

    elif function_of_hex == hex_in_midle_speed:
        in_midle_answers.append(key_resp_2.keys)
...
results = DataFrame({'in_hight': in_hight_answers,'in_midle': in_midle_answers, 'in_low': in_low_answers, 'out_hight' : out_hight_answers,'out_midle' : out_midle_answers, 'out_low' : out_low_answers})
